  ...
  return <div className="min-h-screen">
    <header className="sticky top-0 z-10 border-b border-primary-100 bg-white/95 shadow-sm backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4">
        ...
        <nav className="flex items-center gap-4 text-sm"><Link to="/">Sản phẩm</Link>{!user && <Link to="/track-order">Tra cứu đơn</Link>}{user && !internal && <><Link to="/prescriptions">Đơn thuốc</Link><Link to="/orders">Đơn hàng</Link><Link to="/profile">Hồ sơ</Link></>}{internal ? <Link to="/internal" className="rounded-lg bg-primary-600 px-3 py-2 text-white">Về trang nhân viên</Link> : <Link to="/cart" className="rounded-lg bg-primary-600 px-3 py-2 text-white">Giỏ hàng ({cartCount})</Link>}{user ? <><span className="hidden text-slate-500 lg:inline">{user.displayName}</span><button onClick={signOut} className="rounded-lg bg-primary-50 px-3 py-2 text-primary-700">Đăng xuất</button></> : <><Link className="font-semibold text-primary-700" to="/login">Đăng nhập</Link><Link className="hidden rounded-lg border border-primary-200 px-3 py-2 text-primary-700 sm:inline" to="/register">Đăng ký</Link></>}</nav>
      </div>
    </header>
