...
export default function Orders() {
  ...
  useEffect(() => { const load = () => client.get<PagedResponse<OrderResponse>>('/orders', { params: { status: status || undefined, pageSize: 100 } }).then((response) => setOrders(response.data.items)).catch((reason) => setError(reason.message)); load(); const timer = window.setInterval(load, 15000); return () => window.clearInterval(timer) }, [status])
